...
PauseScreen::PauseScreen(float window_side, float space)
{
...
void PauseScreen::MainHeadLine()
{
    ...
    m_main_headline.setString("Pause");
    m_main_headline.setPosition(sf::Vector2f(m_window_side * 0.43, 6 * m_space));
...
void PauseScreen::PressTo()
{
    ...
    m_press_to_start.setString("Press SPACE to resume");
    m_press_to_start.setPosition(sf::Vector2f(m_window_side * 0.32, 12 * m_space));
    m_press_to_start.setCharacterSize(32);
    m_press_to_start.setFillColor(sf::Color::Red);
    m_press_to_start.setStyle(sf::Text::Bold);
}

void PauseScreen::Draw(sf::RenderWindow &window_instance)
{
